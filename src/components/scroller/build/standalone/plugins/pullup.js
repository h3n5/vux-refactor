    ...
    var Util = {
      ...
      findParentEl: function (el, selector, rootNode) {
        ...
        while (!rs) {
          ...
          if (!parent) break
          if (
            (type == 'class' && parent.className && parent.className.match(sel)) ||
            (type == 'id' && parent.id && _trim(parent.id) == sel) ||
            (type == 'tag' && parent.tagName && parent.tagName.toLowerCase() == sel)
          ) {
            rs = parent
            return rs
            break
  ...
  plugins_pullup = (function (exports) {
    ...
    Util.extend(PullUp, Base, {
      ...
      _scrollHandler: function (e) {
        ...
        if (
          !self.isLoading &&
          Math.abs(e.scrollTop) + xscroll.height + self.userConfig.height + self.userConfig.bufferHeight >=
            xscroll.containerHeight + xscroll.boundry._xtop + xscroll.boundry._xbottom
        ) {
          self._changeStatus('loading')
        }
        return self
