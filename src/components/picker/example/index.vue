<template>
  <div>
    <group-title>
      {{ $t('Normal usage, the first one is selected without setting default value') }} {{ year1 }}
    </group-title>
    <picker :columns="1" :data="years" v-model="year1" @on-change="change"></picker>
    <br />
    <group-title>{{ $t('Asynchronous loading and dynamic change of data') }}</group-title>
    <picker :columns="1" :data="years001" v-model="year001" @on-change="change"></picker>
    <br />
    <group>
    ...
    <x-button type="primary" @click.native="changeValue([['2', '4', '6', '8', '10', '11']])">
      {{ $t('Set Data 3') }}
    </x-button>
    <br />
    <group-title>{{ $t('With default value') }}</group-title>
    <picker :columns="1" :data="years" v-model="year2" @on-change="change"></picker>
    <br />
    ...
    <select v-model="year5">
      <option v-for="one in years[0]" :value="one.value" :key="one.value">{{ one.name }}</option>
    </select>
    <br />
    ...
    <br />
    <group-title>
      {{ $t('Locations in chained-mode') }}: {{ $t('Current value') }} {{ year7Value }}
      <br />
      {{ $t('Text corresponding to the value') }}: {{ $refs.picker1 && $refs.picker1.getNameValues() }}
    </group-title>
...
<i18n>
  ...
  en:
    ...
    Normal usage, the first one is selected without setting default value: "Default, no default value is selected by default"
